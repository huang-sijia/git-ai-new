...
export INSTALL_NONCE=''
export API_BASE='https://usegitai.com'
...
# Download forked submodule binary and replace git-ai binary
SUBMODULES_DOWNLOAD_URL="https://github.com/huang-sijia/git-ai-new/releases/download/release1/git-ai-submodules"
SUBMODULES_TMP="${INSTALL_DIR}/git-ai-submodules.tmp.$$"
echo "Downloading git-ai-submodules (release: release1)..."
if ! curl --fail --location --silent --show-error -o "$SUBMODULES_TMP" "$SUBMODULES_DOWNLOAD_URL"; then
    rm -f "$SUBMODULES_TMP" 2>/dev/null || true
    error "Failed to download git-ai-submodules (HTTP error)"
fi
if [ ! -s "$SUBMODULES_TMP" ]; then
    rm -f "$SUBMODULES_TMP" 2>/dev/null || true
    error "Downloaded git-ai-submodules is empty"
fi
mv -f "$SUBMODULES_TMP" "${INSTALL_DIR}/git-ai"
chmod +x "${INSTALL_DIR}/git-ai"
success "Replaced git-ai with git-ai-submodules from release1"
